...
def claim_code(db: Session) -> AnonymousCode:
    ...
    next_order = select(func.coalesce(func.max(AnonymousCode.allocation_order), 0) + 1).scalar_subquery()
    for _ in range(20):
        ...
        claimed_id = db.scalar(update(AnonymousCode).where(AnonymousCode.id == candidate_id, AnonymousCode.status == CodeStatus.unused)
            .values(status=CodeStatus.used, allocation_order=next_order, allocated_at=datetime.now(timezone.utc).replace(tzinfo=None)).returning(AnonymousCode.id))
        if claimed_id is not None:
            db.flush(); return db.get(AnonymousCode, claimed_id)
        db.expire_all()
    raise RuntimeError("anonymous code allocation conflict; retry transaction")
